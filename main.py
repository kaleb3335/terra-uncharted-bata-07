...
def on_b_pressed():
    ...
    if inmenu == 0:
        if inbuild == 0:
            inmenu = 1
            myMenu = miniMenu.create_menu(miniMenu.create_menu_item("dirt pick"),
                miniMenu.create_menu_item("dirt axe", assets.image("""
                    dirt axe
                """)),
                miniMenu.create_menu_item("mining stick",
                    img("""
                        . . . . . . . . . . . . . . . . 
                                        . . . . . . e e e e e . . . . . 
                                        . . e . e e . . . . . e e . . . 
            ...
            def on_button_pressed(selection, selectedIndex):
                global dirt_pick, dirt_axe, mstick, inmenu
                if selectedIndex == 0:
                    if dirt_pick == 0:
                        if dirt >= 3 and sticks >= 2:
                            dirt_pick = 1
                            game.show_long_text("Crafted dirt pick", DialogLayout.BOTTOM)
                        else:
                            game.show_long_text("you do not have dirt (3) and sticks (2)",
                                DialogLayout.BOTTOM)
                    elif dirt_pick == 1:
                        dirt_pick = 2
                        game.show_long_text("Equipped dirt pick", DialogLayout.BOTTOM)
                        if dirt_axe == 2:
                            dirt_axe = 1
                            game.show_long_text("unequipped dirt axe", DialogLayout.BOTTOM)
                        elif mstick == 2:
                            mstick = 1
                            game.show_long_text("unequipped mining stick", DialogLayout.BOTTOM)
                elif selectedIndex == 1:
                    if dirt_axe == 0:
                        if dirt >= 3 and sticks >= 2:
                            dirt_axe = 1
                            game.show_long_text("Crafted dirt axe", DialogLayout.BOTTOM)
                        else:
                            game.show_long_text("you do not have dirt (3) and sticks (2)",
                                DialogLayout.BOTTOM)
                    elif dirt_axe == 1:
                        dirt_axe = 2
                        game.show_long_text("Equipped dirt axe", DialogLayout.BOTTOM)
                        if dirt_pick == 2:
                            dirt_pick = 1
                            game.show_long_text("unequipped dirt pick", DialogLayout.BOTTOM)
                        elif mstick == 2:
                            mstick = 1
                            game.show_long_text("unequipped mining stick", DialogLayout.BOTTOM)
                    ...
                    if mstick == 0:
                        if sticks >= 3:
                            mstick = 1
                            game.show_long_text("Crafted mining stick", DialogLayout.BOTTOM)
                        else:
                            game.show_long_text("you do not have sticks (3)", DialogLayout.BOTTOM)
                    elif mstick == 1:
                        mstick = 2
                        game.show_long_text("Equipped mining stick", DialogLayout.BOTTOM)
                        if dirt_pick == 2:
                            dirt_pick = 1
                            game.show_long_text("unequipped dirt pick", DialogLayout.BOTTOM)
                        elif dirt_axe == 2:
        ...
        if inbuild == 1:
            ...
            myMenu = miniMenu.create_menu(miniMenu.create_menu_item("dirt", assets.tile("""
                ...
                miniMenu.create_menu_item("wood floor", assets.tile("""
                    myTile18
                """)),
                miniMenu.create_menu_item("wood cling", assets.tile("""
                    myTile23
                """)))
            ...
            def on_button_pressed2(selection2, selectedIndex2):
                ...
                if selectedIndex2 == 0:
                    ...
                    if wood >= 1:
                        ...
                        game.show_long_text("you do not have a crafting table", DialogLayout.BOTTOM)
...
def make_trees(num: number):
    ...
    for value113 in tiles.get_tiles_by_type(assets.tile("""
        myTile69
    """)):
        if tiles.tile_at_location_equals(value113.get_neighboring_location(CollisionDirection.TOP),
            assets.tile("""
                transparency16
            """)):
            tiles.set_tile_at(value113.get_neighboring_location(CollisionDirection.TOP),
                assets.tile("""
                    myTile68
                """))
    ...
    for value114 in tiles.get_tiles_by_type(assets.tile("""
        myTile80
    """)):
        if tiles.tile_at_location_equals(value114.get_neighboring_location(CollisionDirection.RIGHT),
            assets.tile("""
                transparency16
            """)):
            tiles.set_tile_at(value114.get_neighboring_location(CollisionDirection.RIGHT),
                assets.tile("""
                    myTile78
                """))
    for value115 in tiles.get_tiles_by_type(assets.tile("""
        myTile80
    """)):
        if tiles.tile_at_location_equals(value115.get_neighboring_location(CollisionDirection.LEFT),
            assets.tile("""
                transparency16
            """)):
            tiles.set_tile_at(value115.get_neighboring_location(CollisionDirection.LEFT),
                assets.tile("""
                    myTile77
                """))
    ...
    for value119 in tiles.get_tiles_by_type(assets.tile("""
        myTile80
    """)):
        if tiles.tile_at_location_equals(value119.get_neighboring_location(CollisionDirection.TOP),
            assets.tile("""
                transparency16
            """)):
            tiles.set_tile_at(value119.get_neighboring_location(CollisionDirection.TOP),
                assets.tile("""
                    myTile81
                """))
    for value1110 in tiles.get_tiles_by_type(assets.tile("""
        myTile81
    """)):
        if tiles.tile_at_location_equals(value1110.get_neighboring_location(CollisionDirection.RIGHT),
            assets.tile("""
                transparency16
            """)):
            tiles.set_tile_at(value1110.get_neighboring_location(CollisionDirection.RIGHT),
                assets.tile("""
                    myTile83
                """))
    for value1111 in tiles.get_tiles_by_type(assets.tile("""
        myTile81
    """)):
        if tiles.tile_at_location_equals(value1111.get_neighboring_location(CollisionDirection.LEFT),
            assets.tile("""
                transparency16
            """)):
            tiles.set_tile_at(value1111.get_neighboring_location(CollisionDirection.LEFT),
                assets.tile("""
                    myTile82
                """))
    ...
    if controller.A.is_pressed():
        if inbuild == 0:
            if inmenu == 0:
                ...
                def on_throttle():
                    global blk_stone, inmenu, myMenu
                    blk_stone = 15
                    inmenu = 1
                    myMenu = miniMenu.create_menu(miniMenu.create_menu_item("craft stone wall", assets.tile("""
                            myTile8
                        """)),
                        miniMenu.create_menu_item("Craft blk stone wall", assets.tile("""
                            myTile7
                        """)),
                        miniMenu.create_menu_item("craft wooden backing", assets.tile("""
                            myTile17
                        """)),
                    ...
                    def on_button_pressed3(selection5, selectedIndex5):
                        global stone_wall, stone, blk_stone_wall, blk_stone, wdwall, wood, wdfloor, wdciling
                        if selectedIndex5 == 0:
                            if stone >= 15:
                                stone_wall += 5
                                stone += -15
                            ...
                            if wood >= 15:
                                ...
                                game.show_long_text("you do not have wood (15)", DialogLayout.BOTTOM)
                        elif selectedIndex5 == 4:
                            if wood >= 15:
                                wdciling += 5
                                wood += -15
                            else:
                                game.show_long_text("you do not have wood wood (15)", DialogLayout.BOTTOM)
                        sprites.destroy_all_sprites_of_kind(SpriteKind.mini_menu)
...
def makeplayer():
    global player1, select, playerhealth, world_gen_done
    ...
    playerhealth.attach_to_sprite(player1)
    world_gen_done = 1
def make_underworld():
    global col
    statusbar.value += load_p
    ...
    for index7 in range(16):
        tiles.set_tile_at(tiles.get_tile_location(col, 239),
            assets.tile("""
                myTile3
            """))
        col += 16
    for index8 in range(16):
            ...
            if Math.percent_chance(15):
                tiles.set_tile_at(value1132.get_neighboring_location(CollisionDirection.BOTTOM),
                    assets.tile("""
                        transparency16
                    """))
    for index10 in range(25):
        for value132 in tiles.get_tiles_by_type(assets.tile("""
            transparency16
        """)):
            if Math.percent_chance(randint(0, 40)):
                ...
                if tiles.tile_at_location_equals(value132.get_neighboring_location(CollisionDirection.TOP),
                    ...
                    tiles.set_tile_at(value132.get_neighboring_location(CollisionDirection.TOP),
                        assets.tile("""
                            transparency16
                        """))
            elif Math.percent_chance(randint(0, 40)):
                ...
                if tiles.tile_at_location_equals(value132.get_neighboring_location(CollisionDirection.BOTTOM),
                    ...
                    tiles.set_tile_at(value132.get_neighboring_location(CollisionDirection.BOTTOM),
                        ...
                        """))
    col = 0
    for index11 in range(255):
        col += 1
        if tiles.tile_at_location_equals(tiles.get_tile_location(col, 234),
            assets.tile("""
                transparency16
            """)):
            tiles.set_tile_at(tiles.get_tile_location(col, 234),
                assets.tile("""
                    myTile50
                """))
    ...
    for index13 in range(2):
        for index14 in range(randint(1, 4)):
...
def world_gen():
    ...
    print(len(tiles.get_tiles_by_type(assets.tile("""
        myTile13
    """))) + (len(tiles.get_tiles_by_type(assets.tile("""
        myTile14
    """))) + (len(tiles.get_tiles_by_type(assets.tile("""
        myTile16
    """))) + (len(tiles.get_tiles_by_type(assets.tile("""
...
def make_caves():
    statusbar.value += load_p
    pause(100)
    for index15 in range(40):
        for index16 in range(randint(0, 4)):
...
def on_menu_pressed():
    ...
    if inmenu == 0:
        ...
        def on_button_pressed4(selection3, selectedIndex3):
            ...
            if selectedIndex3 == 0:
                ...
                if inmenu == 0:
                    ...
                    def on_button_pressed5(selection4, selectedIndex4):
                        global inmenu
                        sprites.destroy_all_sprites_of_kind(SpriteKind.mini_menu)
                        inmenu = 0
                ...
                if inmenu == 0:
                    ...
                    def on_button_pressed6(selection42, selectedIndex42):
                        global myMenu
                        if selectedIndex42 == 0:
                            ...
                            def on_button_pressed7(selection43, selectedIndex43):
                                ...
                                if selectedIndex43 == 0:
                                    ...
                                    inmenu = 0
                                    lighting = 0
                            myMenu.on_button_pressed(controller.A, on_button_pressed7)
                            
                        else:
                            pass
                    myMenu.on_button_pressed(controller.A, on_button_pressed6)
                    
        gamemenu.on_button_pressed(controller.A, on_button_pressed4)
...
def on_overlap_tile6(sprite5, location5):
    while player1.tilemap_location() == location5:
        sprite5.y += -0.01
    if sprite5.is_hitting_tile(CollisionDirection.RIGHT) or sprite5.is_hitting_tile(CollisionDirection.LEFT):
        sprite5.y += -3
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        myTile62
...
def on_overlap_tile7(sprite23, location23):
...
def on_overlap_tile8(sprite6, location6):
    if location6.column == 0 and (characterAnimations.matches_rule(player1, characterAnimations.rule(Predicate.MOVING_LEFT)) and secworld == 1):
        if tileUtil.current_tilemap() == tilemap_1:
            tiles.set_current_tilemap(tilemap_2)
            tiles.place_on_tile(player1,
                tiles.get_tile_location(254, player1.tilemap_location().row))
            tileUtil.set_tile_at(tilemap_2,
                tiles.get_tile_location(254, player1.tilemap_location().row),
                assets.tile("""
                    transparency16
                """))
            tileUtil.set_tile_at(tilemap_2,
                ...
                assets.tile("""
                    transparency16
                """))
            tileUtil.set_wall_at(tilemap_2,
                tiles.get_tile_location(254, player1.tilemap_location().row),
                False)
            tileUtil.set_wall_at(tilemap_2,
                tiles.get_tile_location(254, player1.tilemap_location().row + 1),
                False)
        elif tileUtil.current_tilemap() == tilemap_2:
            tiles.set_current_tilemap(tile_map_3)
            tiles.place_on_tile(player1,
                tiles.get_tile_location(254, player1.tilemap_location().row))
            tileUtil.set_tile_at(tile_map_3,
                tiles.get_tile_location(254, player1.tilemap_location().row),
                assets.tile("""
                    transparency16
                """))
            tileUtil.set_wall_at(tile_map_3,
                tiles.get_tile_location(254, player1.tilemap_location().row),
                False)
            tileUtil.set_tile_at(tile_map_3,
                tiles.get_tile_location(254, player1.tilemap_location().row + 1),
            ...
            tileUtil.set_wall_at(tile_map_3,
                tiles.get_tile_location(254, player1.tilemap_location().row + 1),
                False)
        elif tileUtil.current_tilemap() == tile_map_3:
            tiles.set_current_tilemap(tilemap_4)
            tiles.place_on_tile(player1,
                tiles.get_tile_location(254, player1.tilemap_location().row))
            tileUtil.set_tile_at(tilemap_4,
                tiles.get_tile_location(254, player1.tilemap_location().row),
                assets.tile("""
                    transparency16
                """))
            tileUtil.set_wall_at(tilemap_4,
                tiles.get_tile_location(254, player1.tilemap_location().row),
                False)
            tileUtil.set_tile_at(tilemap_4,
                tiles.get_tile_location(254, player1.tilemap_location().row + 1),
            ...
            tileUtil.set_wall_at(tilemap_4,
                tiles.get_tile_location(254, player1.tilemap_location().row + 1),
                False)
        elif tileUtil.current_tilemap() == tilemap_4:
            tiles.set_current_tilemap(tilemap_1)
            tiles.place_on_tile(player1,
                tiles.get_tile_location(254, player1.tilemap_location().row))
            tileUtil.set_tile_at(tilemap_1,
                tiles.get_tile_location(254, player1.tilemap_location().row),
                assets.tile("""
                    transparency16
                """))
            tileUtil.set_wall_at(tilemap_1,
                tiles.get_tile_location(254, player1.tilemap_location().row),
                False)
            tileUtil.set_tile_at(tilemap_1,
                tiles.get_tile_location(254, player1.tilemap_location().row + 1),
            ...
            tileUtil.set_wall_at(tilemap_1,
                tiles.get_tile_location(254, player1.tilemap_location().row + 1),
                False)
    elif location6.column == 254 and (characterAnimations.matches_rule(player1, characterAnimations.rule(Predicate.MOVING_RIGHT)) and secworld == 1):
        if tileUtil.current_tilemap() == tilemap_1:
            tiles.set_current_tilemap(tilemap_4)
            tiles.place_on_tile(player1,
...
def make_dirt():
    ...
    if Math.percent_chance(25):
        ...
        for index30 in range(8):
            tiles.set_tile_at(tiles.get_tile_location(col, 64),
                assets.tile("""
                    myTile61
                """))
            col += 16
        for index31 in range(8):
            tiles.set_tile_at(tiles.get_tile_location(col, 64),
                assets.tile("""
                    myTile1
                """))
        ...
        for index34 in range(6):
            tiles.set_tile_at(tiles.get_tile_location(col, 64),
                assets.tile("""
                    myTile1
                """))
            col += 16
        for index35 in range(6):
            tiles.set_tile_at(tiles.get_tile_location(col, 64),
                assets.tile("""
                    myTile64
                """))
...
world_gen_done = 0
player1: Sprite = None
tilemap_4: tiles.TileMapData = None
tile_map_3: tiles.TileMapData = None
tilemap_2: tiles.TileMapData = None
secworld = 0
row = 0
load_p = 0
col = 0
...
def on_button_pressed8(selection6, selectedIndex6):
    global statusbar, tilemap_1, col, load_p, row, secworld, tilemap_2, tile_map_3, tilemap_4
    sprites.destroy_all_sprites_of_kind(SpriteKind.mini_menu)
    if selectedIndex6 == 0:
        ...
        world_gen()
        makeplayer()
myMenu2.on_button_pressed(controller.A, on_button_pressed8)

@namespace
class userconfig:
    ARCADE_SCREEN_WIDTH = 320
    ARCADE_SCREEN_HEIGHT = 240
death_messages = ["\"Well, that was embarrassing. Wanna try again… or just quit now?\"",
    "\"Looks like you're collecting deaths, not wins.\"",
    "\"Remember when you thought you were good at this? Yeah, me neither.\"",
    "Pro tip: You might want to avoid dying next time.\"",
    "\"Is this part of your master plan? Because it looks like failure.\"",
    "\"You make failure look effortless.\"",
    "At least you're consistent… consistently bad.\"",
    "\"No one dies quite like you.\"",
    "\"If you were trying to impress me, it's not working.\"",
    "Have you considered a less challenging hobby? Like watching paint dry?\"",
    "Don't worry, losing is just your style.\"",
    "\"You were so close! Just kidding, no, you weren't.\"",
    "\"On the bright side, no one saw that… except me.\"",
    "\"I think the game just felt sorry for you.\"",
    "\"That's a bold strategy. Let's see if it fails every time.\"",
    "\"You have the reflexes of a sleepy sloth.\"",
    "\"Maybe read the instructions next time?\"",
    "\"Maybe read the instructions next time?\"",
    "\"The controller isn't broken. You are.\"",
    "\"That's one way to lower the difficulty. Just keep dying.\""]

def on_on_update():
    global fall_damage, fall_damage_2
    if world_gen_done == 1:
        if player1.vy >= 300:
            fall_damage = 1
            fall_damage_2 = player1.vy / 25
        if player1.is_hitting_tile(CollisionDirection.BOTTOM) and fall_damage == 1:
...
def on_forever5():
    if world_gen_done == 1:
        if inmenu == 0:
            if tiles.tile_at_location_equals(player1.tilemap_location(),
                ...
                assets.tile("""
                    myTile50
                """)):
                controller.move_sprite(player1, 35, 0)
        ...
        if playerhealth.value == 0:
            game.splash(death_messages._pick_random())
...
def on_forever6():
    global dirt, stone, blk_stone, sticks
    if controller.A.is_pressed():
        if world_gen_done == 1:
            if inbuild == 0:
                if tiles.tile_at_location_equals(select.tilemap_location(), assets.tile("""
                    myTile1
                """)) or tiles.tile_at_location_equals(select.tilemap_location(), assets.tile("""
                    myTile15
                """)):
                ...
                elif tiles.tile_at_location_equals(select.tilemap_location(), assets.tile("""
                    myTile13
                """)):
                    tiles.set_wall_at(select.tilemap_location(), False)
                    select.start_effect(effects.ashes, 200)
                    ...
                    if dirt >= 1:
                        if buildM == 1:
                            ...
                            def on_throttle12():
                                ...
                                select.start_effect(effects.ashes, 200)
                                tiles.set_tile_at(select.tilemap_location(), assets.tile("""
                                    myTile1
                                """))
                                dirt += -1
                    ...
                    if wood >= 1:
                        if buildM == 2:
                            ...
                            def on_throttle13():
                                ...
                                tiles.set_wall_at(select.tilemap_location(), True)
                                select.start_effect(effects.ashes, 200)
                                tileUtil.cover_tile(select.tilemap_location(), assets.tile("""
                                    myTile9
                                """))
                                wood += -1
                    ...
                    if stone_wall >= 1:
                        if buildM == 3:
                            ...
                            def on_throttle14():
                                ...
                                """))
                                stone_wall += -1
                            timer.throttle("action", 500, on_throttle14)
    ...
    if world_gen_done == 1:
        if player1.y <= 3000:
            for value14 in tiles.get_tiles_by_type(assets.tile("""
                myTile1
            """)):
                if tiles.tile_at_location_equals(value14.get_neighboring_location(CollisionDirection.BOTTOM),
                    assets.tile("""
                        transparency16
                    """)):
                    if Math.percent_chance(10):
                        tiles.set_tile_at(value14.get_neighboring_location(CollisionDirection.BOTTOM),
                            assets.tile("""
                                myTile4
                            """))
        ...
        if player1.y >= 3000:
            ...
            """)):
                if tiles.tile_at_location_equals(value143.get_neighboring_location(CollisionDirection.BOTTOM),
                    ...
                    """)):
                    if Math.percent_chance(25):
                        tileUtil.cover_tile(value143.get_neighboring_location(CollisionDirection.BOTTOM),
                            assets.tile("""
                                myTile48
                            """))
            if Math.percent_chance(25):
